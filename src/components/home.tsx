...
const Home = () => {
    ...
    const header_elems = [
        { tag: 'Home', refFunc: () => setPage(0) },
        { tag: 'Skills', refFunc: () => setPage(1) },
        { tag: 'Experience', refFunc: () => setPage(2) },
        { tag: 'Projects', refFunc: () => setPage(3) },
        { tag: 'Education', refFunc: () => setPage(4) },
        { tag: 'Achievements', refFunc: () => setPage(5) },
        { tag: 'Online Courses', refFunc: () => setPage(6) },
    ];
    ...
    return <div className="home-container">
        <div className="header">
            {header_elems.map((item, key) => (
                <div key={`${item.tag}-${key}`} onClick={() => item.refFunc()}>
                    {item.tag}
                </div>
            ))}
        </div>
                    ...
                    <div className="name-contact">
                        ...
                        <div className="contact">
                            {social_icons.map((item, key) => (
                                <div key={`${item[1]}-${key}`} className='resume_download'>
